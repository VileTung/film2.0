...
class subtitle
{
	...
	private function moveSubtitle($file, $language)
	{
		...
		//Check if in DB
		list($rowCount, $result) = sqlQueryi("SELECT `hash` FROM `subtitle` WHERE `imdb` = ? AND `hash` = ?", array("ss",
			$this->imdb,
			$md5File), true);
		...
		if ($rowCount > 0)
		{
			...
			//Remove from DB
			sqlQueryi("DELETE FROM `subtitle` WHERE `imdb` = ? AND `hash` = ?", array("ss",
				$this->imdb,
				$md5File));
		}

		//Insert in DB
		sqlQueryi("INSERT INTO `subtitle` (`imdb`,`hash`,`language`) VALUES (?,?,?)", array("sss",
			$this->imdb,
			$md5File,
			$language));
