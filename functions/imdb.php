...
class imdb
{
	...
	public function getInfo($id)
	{
		...
		//Check for duplicate
		list($rowCount, $result) = sqlQueryi("SELECT `imdb` FROM `imdb` WHERE `imdb` = ?", array("i",
			$id), true);
		...
		else
		{
			...
			if ($content)
			{
				...
				$this->foundData = array("title" => $title,
	...
	public function database()
	{
		...
		//Insert data
		sqlQueryi("INSERT INTO `imdb` (`imdb`,`title`,`description`,`runtime`,`rating`,`release`,`added`) VALUES (?,?,?,?,?,?,?)", array("sssssss",
			$this->imdbId,
		...
		{
			sqlQueryi("INSERT INTO `genres` (`imdb`,`genre`) VALUES (?,?)", array("ss",
				$this->imdbId,
				$genre));
		}
